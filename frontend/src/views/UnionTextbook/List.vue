<template>
  <div class="union-textbook-table">
    <v-data-table
      v-model="selected"
      :headers="headers"
      :items="unionTextbooks && unionTextbooks.rows ? unionTextbooks.rows : []"
      ...
      class="union-textbook-table pl-3 pr-3 pb-3"
      fixed-header
      hide-default-footer
    >
      <template v-if="unionTextbooks && !unionTextbooks.rows" v-slot:no-data>
...
<!--        ></v-simple-checkbox>-->
<!--      </template>-->
      <template v-slot:item.union_textbook.submitted_union_textbook="{ item }">
        <v-simple-checkbox
          color="info"
          v-model="item.union_textbook.submitted_union_textbook"
          @click="checkSubmission(item)"
          :key="item.id"
        ></v-simple-checkbox>
      </template>
      <template v-slot:item.union_textbook.submitted_at="{ item }">
        <span>{{ formatTime(item.union_textbook.submitted_at)}}</span>
      </template>
      <template v-slot:item.union_textbook.class_confirmed="{ item }">
        <span>{{ formatTime(item.union_textbook.class_confirmed)}}</span>
      </template>
      ...
      <template v-slot:footer>
        ...
        >
          ...
          <v-pagination
            v-model="currentPage"
            v-if="unionTextbooks.count"
            :length="unionTextbooks ? Math.ceil(unionTextbooks.count / selectedSize) : 0"
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {text: 'Nộp sổ đoàn', value: 'union_textbook.submitted_union_textbook',},
  ...
  methods: {
    ...mapActions({
      fetchGetUnionTextbooks: 'fetchGetUnionTextbooks',
    ...
    async search() {
      ...
      await this.fetchGetUnionTextbooks(query);
    },
    setQuery() {
      this.searchOptions.forEach(option => {
