<template>
  <div>
    <v-data-table
        v-model="selected"
        :headers="headers"
        :items="aUnionBranchList ? aUnionBranchList : []"
    ...
    >
      <template
          v-if="aUnionBranchList && !aUnionBranchList.length"
          v-slot:no-data
      >
...
import AssociationUnionBranchForm from './AssociationUnionBranchForm';

export default {
  name: 'association-union-branch-list',
  components: {
    AssociationUnionBranchForm,
    ConfirmDialog,
  },
  data() {
    return {
  ...
  computed: {
    ...mapGetters({
      aUnionBranchList: 'getAUnionBranchList',
    }),
  },
  methods: {
    ...mapActions({
      fetchGetAllABranchList: 'fetchGetAllAUnionBranch',
    }),
    deleteItems() {
  ...
  async created() {
    this.loading = true;
    await this.fetchGetAllABranchList();
    this.loading = false;
    console.log(this.aUnionBranchList);
  },
};
</script>
