<template>
  <div>
    <v-data-table
        v-model="selected"
        :headers="headers"
        :items="unionMemberList ? unionMemberList.Students : []"
        ...
        class="union-member-table pl-3 pr-3 pb-3"
        fixed-header
        hide-default-footer
    >
      <template v-if="unionMemberList && !unionMemberList.Students.length" v-slot:no-data>
      ...
      <template v-slot:footer>
        ...
        >
          ...
          <v-pagination
              v-model="currentPage"
              :length="
              unionMemberList ? Math.ceil(unionMemberList.Paging.TotalRecord / selectedSize) : 0
            "
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {text: 'Lớp', value: 'ActivityClass.Name'},
  ...
  computed: {
    ...mapGetters({
      unionMemberList: 'getUnionMemberList',
    }),
  },

  methods: {
    ...mapActions({
      fetchGetUnionMembers: 'fetchGetUnionMembers',
    }),
    ...mapMutations({
    ...
    async handlePageChange() {
      ...
      this.loading = true;
      await this.fetchGetUnionMembers(this.query);
      this.loading = false;
    },
    ...
    async search() {
      ...
      await this.fetchGetUnionMembers(query);
    },
    setQuery() {
      this.searchOptions.forEach(option => {
  ...
  async created() {
    for (let props in this.$props) {
      if (props === 'FirstName' && this[props]) {
        this.selectedOption = this.searchOptions.find(option => option.attribute === 'FirstName').name;
        this.searchText = this[props];
        break;
      }
      if (props === 'ActivityClass' && this[props]) {
        this.selectedOption = this.searchOptions.find(option => option.attribute === 'ActivityClass').name;
        this.searchText = this[props];
        break;
      }
      if (props === 'Email' && this[props]) {
        this.selectedOption = this.searchOptions.find(option => option.attribute === 'Email').name;
        this.searchText = this[props];
        break;
      }
      if (props === 'Phone' && this[props]) {
        this.selectedOption = this.searchOptions.find(option => option.attribute === 'Phone').name;
        this.searchText = this[props];
        break;
      }
    }
